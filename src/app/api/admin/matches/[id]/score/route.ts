...
import { advanceKnockoutBracket, generateFinalBracket } from '@/lib/tournament/final-phase-service';
...
export async function PATCH(
  ...
  try {
    ...
    let autoGeneratedFinal = false;
    if (match.phase === 'group2' && status === 'completed') {
      const totalPhase2Matches = await MatchModel.countDocuments({ phase: 'group2' });
      const completedPhase2Matches = await MatchModel.countDocuments({ phase: 'group2', status: 'completed' });
      
      if (totalPhase2Matches > 0 && totalPhase2Matches === completedPhase2Matches) {
        try {
          await generateFinalBracket(true);
          autoGeneratedFinal = true;
        } catch (error: unknown) {
          console.warn('Auto-generation of final bracket failed:', error instanceof Error ? error.message : 'Unknown error');
        }
      }
    }

    return NextResponse.json({ success: true, data: match, bracketUpdate, autoGeneratedFinal });
  } catch (error: unknown) {
