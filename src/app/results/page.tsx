  ...
  return (
    <div className="space-y-8 animate-in">
      ...
      {results.length === 0 ? (
        ...
        phaseGroups.map((phase) => {
          const rounds = phase.matches.reduce<Record<string, Match[]>>((acc, match) => {
          ...
          return (
            <section id={phase.id} key={phase.id} className="space-y-4 scroll-mt-24">
              ...
              {Object.keys(rounds).length === 0 ? (
                ...
                Object.entries(rounds).map(([round, matches]) => (
                  <div key={`${phase.id}-${round}`} className="space-y-3">
                    <h3 className="text-lg font-semibold text-white/85">{phaseLabels.round} {round}</h3>
